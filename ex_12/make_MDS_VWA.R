...
reynolds_frame <- read.table(file="locus_VWA.mat",fill=TRUE,header=TRUE)
...
# run the MDS
VWA_MDS <- metaMDS(reynolds_dist)

# open PNG device to output graphic in PNG format
png(file="VWA_MDS_plot.png")

# create the plot
plot(VWA_MDS,type="n",xlab="Axis 1", ylab="Axis 2", main="VWA across Gibraltar")

# add the points
points(VWA_MDS,pch=19,col=region_colors)
