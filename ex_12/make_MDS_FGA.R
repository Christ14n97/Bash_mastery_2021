...
reynolds_frame <- read.table(file="locus_FGA.mat",fill=TRUE,header=TRUE)
...
# run the MDS
FGA_MDS <- metaMDS(reynolds_dist)

# open PNG device to output graphic in PNG format
png(file="FGA_MDS_plot.png")

# create the plot
plot(FGA_MDS,type="n",xlab="Axis 1", ylab="Axis 2", main="FGA across Gibraltar")

# add the points
points(FGA_MDS,pch=19,col=region_colors)
