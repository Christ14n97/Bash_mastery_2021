...
reynolds_frame <- read.table(file="locus_TH01.mat",fill=TRUE,header=TRUE)
...
# run the MDS
TH01_MDS <- metaMDS(reynolds_dist)

# open PNG device to output graphic in PNG format
png(file="TH01_MDS_plot.png")

# create the plot
plot(TH01_MDS,type="n",xlab="Axis 1", ylab="Axis 2", main="TH01 across Gibraltar")

# add the points
points(TH01_MDS,pch=19,col=region_colors)
