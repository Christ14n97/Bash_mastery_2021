...
reynolds_frame <- read.table(file="locus_D13S317.mat",fill=TRUE,header=TRUE)
...
# run the MDS
D13S317_MDS <- metaMDS(reynolds_dist)

# open PNG device to output graphic in PNG format
png(file="D13S317_MDS_plot.png")

# create the plot
plot(D13S317_MDS,type="n",xlab="Axis 1", ylab="Axis 2", main="D13S317 across Gibraltar")

# add the points
points(D13S317_MDS,pch=19,col=region_colors)
