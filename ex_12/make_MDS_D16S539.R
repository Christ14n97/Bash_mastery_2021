...
reynolds_frame <- read.table(file="locus_D16S539.mat",fill=TRUE,header=TRUE)
...
# run the MDS
D16S539_MDS <- metaMDS(reynolds_dist)

# open PNG device to output graphic in PNG format
png(file="D16S539_MDS_plot.png")

# create the plot
plot(D16S539_MDS,type="n",xlab="Axis 1", ylab="Axis 2", main="D16S539 across Gibraltar")

# add the points
points(D16S539_MDS,pch=19,col=region_colors)
